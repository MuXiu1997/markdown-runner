...
import { URL } from 'url'
import { Channel } from '../../preload/src/channel'
import { PlanAST } from '/@/planAST'
import type { Plan } from '/@/plan'
import { setKeyboardLayoutToABC } from '/@/utils'
...
ipcMain.handle(Channel.RUN, async (_event, ...args: [string, number]) => {
  const [markdownTemplate, skipSteps] = args
  return new Promise<void>((resolve) => {
    const hotkey = 'Shift+Alt+CommandOrControl+F10'
    const plan: Plan = new PlanAST(markdownTemplate, skipSteps)
    const cleanup = () => {
      globalShortcut.unregister(hotkey)
      ipcMain.removeHandler('abort')
      ipcMain.removeHandler('showSteps')
    }

    ipcMain.handleOnce('abort', () => {
      cleanup()
      resolve()
    })

    ipcMain.handle('showSteps', () => {
      new Notification({
        title: '当前步数',
        body: `当前步数: ${plan.getStepsNumber()}`,
      }).show()
    })

    const hotKeyHandler = async () => {
      globalShortcut.unregister(hotkey)
      setKeyboardLayoutToABC()
      if (!(await plan.next())) {
        cleanup()
        resolve()
      }
      globalShortcut.register(hotkey, hotKeyHandler)
    }
    globalShortcut.register(hotkey, hotKeyHandler)
  })
})
